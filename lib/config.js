...
module.exports = {
  ...
  STORAGE: {
    ...
    minFixes: Number(process.env.STORAGE_MIN_FIXES || 3),    // need ≥ this many fixes to judge "parked"
  },
  ...
  // Raw positions retention (days). Kept small so the fact table plateaus well
  // within free-tier disk — an unbounded 30d table filled the disk and
  // crash-looped Postgres (see the 2026-07 rebuild). Raise only with headroom.
  POSITIONS_RETENTION_DAYS: Number(process.env.POSITIONS_RETENTION_DAYS || 7),
};
