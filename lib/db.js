...
async function getAllTracks(windowMinutes, hours) {
  const { rows } = await query(
    `SELECT p.mmsi, p.lat, p.lng, p.ts
     FROM positions p
...
async function getCordonSummary(hours, maxTransitHours) {
  ...
  const med = await query(
    `SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY transit_min) AS m
     FROM passages WHERE exit_ts > now() - make_interval(hours => $1::int)`,
    [hours]
  );
  out.completed.medianTransitMin =
    med.rows[0].m != null ? Math.round(med.rows[0].m) : null;
