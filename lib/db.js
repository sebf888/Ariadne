...
function getPool() {
  if (!pool) {
    ...
    // Managed poolers (Supabase pgbouncer) close idle server connections
    // routinely; pg surfaces that as an 'error' on an idle client. With no
    // listener Node treats it as an unhandled 'error' event and crashes the
    // process — which is what silently killed the 24/7 ingest worker. Log and
    // move on; the pool discards the dead client and the next query opens a new
    // one transparently.
    pool.on('error', (err) => {
...
// Denormalized current-position columns carried on each vessel row (see
// schema.sql). buildSnapshot attaches these from the vessel's latest position.
const CUR_POS_COLS = [
  'cur_lat', 'cur_lng', 'cur_sog', 'cur_cog', 'cur_hdt',
  'cur_rot', 'cur_draught', 'cur_status', 'cur_dest', 'cur_eta', 'cur_ts',
];

// Bulk upsert of vessels via UNNEST. Returns { text, values }.
function buildVesselUpsert(vessels) {
  const cols = ['mmsi', 'imo', 'name', 'type', 'flag', 'a', 'b', 'c', 'd', 'last_seen', ...CUR_POS_COLS];
  const values = cols.map((c) => vessels.map((v) => v[c]));
  // Advance the denormalized position only when this report is strictly newer
  // than what's stored, so an out-of-order or backfill write can't rewind it.
  const newer = 'EXCLUDED.cur_ts IS NOT NULL AND (vessels.cur_ts IS NULL OR EXCLUDED.cur_ts > vessels.cur_ts)';
  const curSet = CUR_POS_COLS
    .map((c) => `      ${c} = CASE WHEN ${newer} THEN EXCLUDED.${c} ELSE vessels.${c} END`)
    .join(',\n');
  const text = `
    INSERT INTO vessels (mmsi, imo, name, type, flag, a, b, c, d, last_seen,
      cur_lat, cur_lng, cur_sog, cur_cog, cur_hdt, cur_rot, cur_draught,
      cur_status, cur_dest, cur_eta, cur_ts)
    SELECT * FROM unnest(
      $1::bigint[], $2::bigint[], $3::text[], $4::text[], $5::text[],
      $6::int[], $7::int[], $8::int[], $9::int[], $10::timestamptz[],
      $11::float8[], $12::float8[], $13::float8[], $14::float8[], $15::int[],
      $16::float8[], $17::float8[], $18::int[], $19::text[], $20::text[], $21::timestamptz[]
...
// each with its latest known position. Presence is driven by last_seen
// (ingestion time); the position (cur_ts) may be an older AIS report.
//
// Reads the denormalized cur_* columns on the dimension row — one row per
// vessel — rather than computing DISTINCT ON over the positions fact table.
// That JOIN scanned >1M rows per call and timed out once Gulf-wide tiling grew
// the data; this scans ~12k vessel rows (single-digit ms). cur_* is kept fresh
// by buildVesselUpsert and seeded by scripts/backfill-current-pos.js.
async function getLiveVessels(windowMinutes) {
  const { rows } = await query(
    `SELECT mmsi, imo, name, type, flag, length,
        cur_lat AS lat, cur_lng AS lng, cur_cog AS cog, cur_sog AS sog,
        cur_hdt AS hdt, cur_rot AS rot, cur_draught AS draught,
        cur_status AS status, cur_dest AS dest, cur_eta AS eta, cur_ts AS ts
     FROM vessels
     WHERE last_seen > now() - make_interval(mins => $1::int)
       AND cur_ts IS NOT NULL
     ORDER BY cur_ts DESC`,
    [windowMinutes]
  );
  return rows;
}
...
// cycle; only fills tanker rows still missing an estimate. The maths lives in
// lib/estimate.js (pure, unit-tested, clamped) rather than inline SQL, so it's
// the single documented source of truth and can't silently drift. The upper
// length guard (450 m) drops mislabelled non-tanker hulls before they get a
// crude capacity.
async function enrichVessels(f) {
  const { rows } = await query(
    `SELECT mmsi, length FROM vessels
     WHERE (dwt IS NULL OR size_class IS NULL) AND type ILIKE '%tanker%'
       AND length >= $1::int AND length <= 450`,
    [f.minLengthM]
  );
  const mmsi = [], dwt = [], cap = [], design = [], klass = [];
  for (const r of rows) {
    const d = estimate.estimateDwt(r.length, f);
    if (d == null) continue;
    mmsi.push(r.mmsi);
    dwt.push(d);
    cap.push(estimate.capacityBbl(r.length, f));
    design.push(estimate.designDraught(r.length, f));
    klass.push(estimate.vesselClass(r.length));
  }
  if (!mmsi.length) return 0;
  const res = await query(
    `UPDATE vessels v SET dwt = u.dwt, cargo_capacity_bbl = u.cap,
        design_draught = u.design, size_class = u.klass
     FROM unnest($1::bigint[], $2::bigint[], $3::bigint[], $4::float8[], $5::text[])
          AS u(mmsi, dwt, cap, design, klass)
     WHERE v.mmsi = u.mmsi`,
    [mmsi, dwt, cap, design, klass]
  );
  return res.rowCount;
}
...
async function getFlowSummary(hours, f) {
  const { rows } = await query(
    `WITH calc AS (
       SELECT pa.direction,
              (pa.draught IS NOT NULL AND v.design_draught > 0) AS has_draught,
              (v.cargo_capacity_bbl IS NOT NULL)                AS has_cap,
     ...
     SELECT direction,
            ...
            count(*) FILTER (WHERE has_draught)::int AS with_draught,
            count(*) FILTER (WHERE has_cap)::int     AS with_capacity,
  ...
  const byDir = {};
  for (const r of rows) byDir[r.direction] = r;
  // Each direction carries its own coverage so a reader can weight the estimate:
  // ladenFromDraught vs (tankerPassages − ladenFromDraught) on the fallback.
  const dir = (r) => ({
    tankerPassages: r ? r.tanker_passages : 0,
    barrels: r ? Number(r.barrels) : 0,
    ladenFromDraught: r ? r.with_draught : 0,
    capacityKnown: r ? r.with_capacity : 0,
  });

  // Size-class breakdown, and the crude/product grade PROXY rolled up from it
  // (the feed declares no cargo grade — this is size-based, see estimate.cargoProxy).
  const brk = await query(
    `WITH calc AS (
       SELECT v.size_class AS class, (v.length >= $5::int) AS is_crude,
              COALESCE(v.cargo_capacity_bbl, 0) AS cap,
              COALESCE(
                CASE WHEN pa.draught IS NOT NULL AND v.design_draught > 0
                  THEN GREATEST(0, LEAST(1,
       ...
       FROM passages pa JOIN vessels v ON v.mmsi = pa.mmsi
       WHERE pa.exit_ts > now() - make_interval(hours => $1::int)
         AND pa.type ILIKE '%tanker%'
     )
     SELECT COALESCE(class, 'Unknown') AS class, is_crude,
            count(*)::int AS passages,
            round(COALESCE(sum(cap * laden_frac), 0))::bigint AS barrels
     FROM calc GROUP BY class, is_crude ORDER BY barrels DESC`,
    [hours, f.ballastFraction, f.fallbackLadenOut, f.fallbackLadenIn, estimate.CRUDE_MIN_LEN]
  );
  const byClass = brk.rows.map((r) => ({
    class: r.class, passages: r.passages, barrels: Number(r.barrels),
  }));
  const grade = { crude: { passages: 0, barrels: 0 }, product: { passages: 0, barrels: 0 } };
  for (const r of brk.rows) {
    const g = r.is_crude ? grade.crude : grade.product;
    g.passages += r.passages;
    g.barrels += Number(r.barrels);
  }

  return {
    hours,
    basis: 'detected',
    outbound: dir(byDir.outbound),
    inbound: dir(byDir.inbound),
    byClass,
    byGrade: grade,
    gradeBasis: 'size-proxy',
    assumptions: {
      capacity: `DWT≈${f.refDwt}·(L/${f.refLen})³ clamped [${estimate.DWT_MIN},${estimate.DWT_MAX}]t, ×${f.cargoFraction} cargo ×${f.bblPerTonne} bbl/t`,
      ladenFallback: `draught→laden where reported; else out ${f.fallbackLadenOut}, in ${f.fallbackLadenIn}`,
    },
    note: 'Barrels are ESTIMATES over DETECTED transits only. A transit is counted '
        + 'only when a vessel crosses both cordon gates while under AIS observation, '
        + 'so capture is partial and absolute levels understate true throughput — '
        + 'read as a relative index / trend, not an absolute total. See METHODOLOGY.md.',
  };
}
...
async function getFloatingStorage(windowMinutes, hours, st, flow) {
  const { rows } = await query(
    `WITH win AS (
     ...
     LIMIT 400`,
    [windowMinutes, hours, st.minHours, st.maxSpanKm, st.maxAvgSog, flow.ballastFraction, st.minFixes]
  );
...
// --- Persistent daily rollups (analysis that must outlive the 7-day prune) ---
//
// Snapshot one UTC day's crude-market signals into the daily_* tables. Uses the
// same barrel/laden model as the live endpoints (getFlowSummary /
// getFloatingStorage) so history and live agree. Idempotent — an upsert (and a
// per-vessel DELETE+INSERT) fully rewrites the day, so catch-up and backfill are
// safe to re-run. Only meaningful for days whose positions still exist (< 7d).
async function rollupDay(dateStr, flow, storage) {
  // 1) Strait barrel flow by direction, from persisted passages. Mirrors
  //    getFlowSummary exactly; also records draught coverage per day.
  await query(
    `INSERT INTO daily_flows (d, direction, passages, with_draught, barrels)
     SELECT $1::date, direction, count(*)::int,
            count(*) FILTER (WHERE has_draught)::int,
            round(COALESCE(sum(cap * laden_frac), 0))::bigint
     FROM (
       SELECT pa.direction,
              (pa.draught IS NOT NULL AND v.design_draught > 0) AS has_draught,
              COALESCE(v.cargo_capacity_bbl, 0) AS cap,
       ...
       FROM passages pa JOIN vessels v ON v.mmsi = pa.mmsi
       WHERE pa.type ILIKE '%tanker%'
         AND pa.exit_ts >= $1::date AND pa.exit_ts < ($1::date + 1)
     ) calc
     GROUP BY direction
     ON CONFLICT (d, direction) DO UPDATE SET
       passages = EXCLUDED.passages, with_draught = EXCLUDED.with_draught,
       barrels = EXCLUDED.barrels, computed_at = now()`,
    [dateStr, flow.ballastFraction, flow.fallbackLadenOut, flow.fallbackLadenIn]
  );

  // 2) Floating storage: tankers that sat effectively still that day. Rebuild
  //    the per-vessel membership exactly (DELETE first so a re-run can't leave
  //    stale rows), then aggregate the day from it.
  await query(`DELETE FROM daily_storage_vessel WHERE d = $1::date`, [dateStr]);
  await query(
    `INSERT INTO daily_storage_vessel (d, mmsi, parked_hours, barrels)
     WITH win AS (
       SELECT p.mmsi, p.lat, p.lng, p.sog, p.ts, p.draught
       FROM positions p JOIN vessels v ON v.mmsi = p.mmsi
       WHERE v.type ILIKE '%tanker%'
         AND p.ts >= $1::date AND p.ts < ($1::date + 1)
     ),
     agg AS (
       SELECT mmsi, count(*)::int AS n, avg(sog) AS avg_sog,
              EXTRACT(EPOCH FROM (max(ts) - min(ts))) / 3600.0 AS span_hours,
     ...
     latest AS (SELECT DISTINCT ON (mmsi) mmsi, draught FROM win ORDER BY mmsi, ts DESC)
     SELECT $1::date, a.mmsi, round(a.span_hours::numeric, 1)::real,
            CASE WHEN l.draught IS NOT NULL AND v.design_draught > 0
                      AND v.cargo_capacity_bbl IS NOT NULL
              THEN round(v.cargo_capacity_bbl * GREATEST(0, LEAST(1,
                   (l.draught - $5::float8 * v.design_draught)
                   / NULLIF((1 - $5::float8) * v.design_draught, 0))))::bigint
            END
     FROM agg a
     JOIN vessels v ON v.mmsi = a.mmsi
     JOIN latest l ON l.mmsi = a.mmsi
     WHERE a.span_hours >= $2::float8 AND a.bbox_km <= $3::float8
       AND (a.avg_sog IS NULL OR a.avg_sog <= $4::float8)
       AND a.n >= $6::int`,
    [dateStr, storage.minHours, storage.maxSpanKm, storage.maxAvgSog, flow.ballastFraction, storage.minFixes]
  );
  await query(
    `INSERT INTO daily_storage (d, parked, laden_count, barrels, avg_parked_hours)
     SELECT $1::date, count(*)::int, count(barrels)::int,
            COALESCE(sum(barrels), 0)::bigint, round(avg(parked_hours)::numeric, 1)::real
     FROM daily_storage_vessel WHERE d = $1::date
     ON CONFLICT (d) DO UPDATE SET
       parked = EXCLUDED.parked, laden_count = EXCLUDED.laden_count,
       barrels = EXCLUDED.barrels, avg_parked_hours = EXCLUDED.avg_parked_hours,
       computed_at = now()`,
    [dateStr]
  );
}

// Roll up every complete UTC day not yet captured (self-healing catch-up if the
// worker was down), bounded so a long gap can't stall one cycle. Days strictly
// before today; today is still accumulating. daily_storage.d is the watermark.
async function rollupPendingDays(flow, storage, maxDays = 10) {
  const { rows } = await query(
    `WITH bounds AS (
       SELECT (SELECT min(ts)::date FROM positions)  AS first_pos,
              (SELECT max(d)          FROM daily_storage) AS last_roll
     )
     SELECT to_char(gs::date, 'YYYY-MM-DD') AS d
     FROM bounds, generate_series(
       COALESCE(bounds.last_roll + 1, bounds.first_pos),
  ...
  const done = [];
  for (const r of rows) { await rollupDay(r.d, flow, storage); done.push(r.d); }
  return done;
}

// Persistent history for the dashboard: daily flow + floating-storage series,
// plus current floating-storage dwell buckets (tankers continuously parked
// >= 7 / 12 / 20 days as of the latest snapshot — the Vortexa-style tell).
async function getHistory(days) {
  const [flows, storage, dwell] = await Promise.all([
    query(
      `SELECT to_char(d, 'YYYY-MM-DD') AS d, direction, passages, with_draught, barrels
       FROM daily_flows
       WHERE d > (now() AT TIME ZONE 'UTC')::date - $1::int
       ORDER BY d, direction`, [days]),
    query(
      `SELECT to_char(d, 'YYYY-MM-DD') AS d, parked, laden_count, barrels, avg_parked_hours
       FROM daily_storage
       WHERE d > (now() AT TIME ZONE 'UTC')::date - $1::int
       ORDER BY d`, [days]),
    query(
      `WITH runs AS (
         SELECT mmsi, d,
                d - (row_number() OVER (PARTITION BY mmsi ORDER BY d))::int AS grp
         FROM daily_storage_vessel
       ),
       run_len AS (
         SELECT mmsi, count(*) AS dwell_days, max(d) AS last_d
         FROM runs GROUP BY mmsi, grp
       )
       SELECT
         count(*) FILTER (WHERE dwell_days >= 7)  AS d7,
         count(*) FILTER (WHERE dwell_days >= 12) AS d12,
         count(*) FILTER (WHERE dwell_days >= 20) AS d20
       FROM run_len
       WHERE last_d = (SELECT max(d) FROM daily_storage_vessel)`),
  ]);
  const dw = dwell.rows[0] || {};
  return {
    days,
    flows: flows.rows.map((r) => ({ ...r, barrels: Number(r.barrels) })),
    storage: storage.rows.map((r) => ({ ...r, barrels: Number(r.barrels) })),
    dwell: { d7: Number(dw.d7 || 0), d12: Number(dw.d12 || 0), d20: Number(dw.d20 || 0) },
  };
}
