...
async function rollupDay(dateStr, flow, storage) {
  // 1) Strait barrel flow by direction, from persisted passages.
  await query(
    `INSERT INTO daily_flows (d, direction, passages, barrels)
     ...
     ON CONFLICT (d, direction) DO UPDATE SET
       passages = EXCLUDED.passages, barrels = EXCLUDED.barrels, computed_at = now()`,
  ...
  await query(
    `INSERT INTO daily_storage_vessel (d, mmsi, parked_hours, barrels)
     ...
     agg AS (
       SELECT mmsi, avg(sog) AS avg_sog,
              EXTRACT(EPOCH FROM (max(ts) - min(ts))) / 3600.0 AS span_hours,
              ST_DistanceSphere(ST_MakePoint(min(lng), min(lat)),
                                ST_MakePoint(max(lng), max(lat))) / 1000.0 AS bbox_km
       FROM win GROUP BY mmsi
    ...
    [dateStr, storage.minHours, storage.maxSpanKm, storage.maxAvgSog, flow.ballastFraction]
  );
  await query(
...
async function getHistory(days) {
  const [flows, storage, dwell] = await Promise.all([
    query(
      `SELECT to_char(d, 'YYYY-MM-DD') AS d, direction, passages, barrels
       FROM daily_flows
