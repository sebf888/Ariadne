...
async function enrichVessels(f) {
  const { rows } = await query(
    `SELECT mmsi, length FROM vessels
     WHERE dwt IS NULL AND type ILIKE '%tanker%'
       AND length >= $1::int AND length <= 450`,
    [f.minLengthM]
  );
  const mmsi = [], dwt = [], cap = [], design = [];
  for (const r of rows) {
  ...
  const res = await query(
    `UPDATE vessels v SET dwt = u.dwt, cargo_capacity_bbl = u.cap, design_draught = u.design
     FROM unnest($1::bigint[], $2::bigint[], $3::bigint[], $4::float8[])
          AS u(mmsi, dwt, cap, design)
     WHERE v.mmsi = u.mmsi`,
    [mmsi, dwt, cap, design]
  );
  return res.rowCount;
